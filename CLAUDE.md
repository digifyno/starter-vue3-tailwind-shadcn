<!-- rsi-worker-metadata
  workerId=eb07c458-1c1f-43ea-a3cf-7460aa268f80
  productId=ced92709-47c9-484d-bcaa-2b2c4a90faed
  scopeConfigHash=06cbc815be080494
  generatedAt=2026-04-28T04:52:28.162Z
-->
...
Currently covered: dark mode toggle interaction (including icon-swap — toggle button swaps SVG icon sun↔moon on click), localStorage persistence, light mode fallback (defaults to light mode when localStorage is empty and no dark class on `<html>`), skip-to-content link, `<main>` landmark, SVG `aria-hidden`, `aria-pressed` state, prefers-reduced-motion behavior, error tracking (no-op in dev/missing-token/missing-URL, no console noise on no-op path, POST to Hub API with auth header and error payload, optional context for Vue errorHandler and unhandledrejection paths, silent fetch failure, silent non-ok HTTP response — no throw and no console noise when fetch resolves with 4xx/5xx, trailing slash normalization — `VITE_HUB_URL` trailing slash stripped when constructing the endpoint URL), `lang="en"` on `<html>`, initial `class="dark"` on `<html>`, dynamic aria-label update, keyboard accessibility, h1 rendering (h1 element exists and contains expected text), feature items rendering (elements with `data-testid="feature-item"` are present), documentation link attributes (`rel="noopener noreferrer"`, `target="_blank"`, and `aria-label` includes "opens in new tab"), external links security (all `target="_blank"` links carry `rel` containing `noopener` and `noreferrer`), App axe accessibility (no violations in light mode and dark mode), WelcomeCard rendering (title, description and default slot simultaneously, default/footer/header slots, all three named slots simultaneously — header/default/footer slots all render at once when all are provided, header slot priority — header slot content renders and title prop `<h2>` is suppressed when both are provided, footer-absent branch — footer section omitted when no footer slot is provided, header-absent branch, description-absent branch — `<p>` element omitted when description prop is not provided), WelcomeCard accessibility (aria-disabled attribute — set to `"true"` when disabled, absent when enabled; inert attribute — present when disabled, absent when enabled; disabled visual state, pointer-events-none on disabled — `pointer-events-none` class applied when `disabled` prop is true), and WelcomeCard axe accessibility (no violations in default/disabled/empty states). Note: CSP meta tag cannot be tested in jsdom (index.html is not parsed); verify via index.html inspection or e2e tests.
